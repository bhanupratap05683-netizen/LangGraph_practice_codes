# Learning LangGraph and making project files , here i am practicing langGraph and also making practice file for that.
--------------------------------------------------------------------------------------
## LangGraph practice Codes
## Practice code.py - Files context
- Day 1 practice code.py = using a basic langgraph invoke function to call AI response.
- Day 2 practice code.py = use of embbeding function to make embbedings of a sentence,document.
- Day 3 practice code.py = build a chatbot with the help of langchain. , and made a UI for it to look interactive.
- Day 4 practice code.py = PDF loader and make a summary on the given docs.
- Day 5 practice code.py = Website scraper and write a summary on that webstie.
- Day 6 practice code.py = chatbot which has ability to extract the info from give text it's a resume praser chat bot.
- Day 7 practice code.py = extract data from resume and gives JASON output.
- Day 8 practice code.py = locally prase the resume.
- Day 9 practice code.py = locally prase resume with advance contact info extraction method.
- Day 10 practice code.py = job description (JD) baises resume prasing.
- Day 11 practice code.py = improved version of every resume praser.

---------------------------------------------------------------------------------------

## UI.py - Files
- UI.py = UI for day 3 practice code
- UI2.py = upgraded UI for day 3 practice code plus add different modes for chat.
- UI3.py = AI resume praser which gives JASON output.
- UI4.py = AI resume praser which gives the direct output of the pdfs important info.
- UI5.py = Resume prasser which prase according to the given job description.
- UI6.py = on progress 

---------------------------------------------------------------------------------------

```
PDFs
- GRU.pdf = pdf file for day 4 work.
- Aman Sharma aman.pdf = demo pdf file to practice day 8 pracctice code.

